...
import {BaseFilter} from 'src/app/shared/models/Paging';
...
export class GeneralUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      // If the title is 'showSearch', display the search bar
      if (title === 'showSearch') {
  ...
  viewBookDetails(book: any) {
    this.router.navigate([
      `dashboard/user-profile/${book.id}`,
      {
  ...
  viewAuthrDetails(auther: any) {
    this.router.navigate([
      `dashboard/auther/${auther.id}`,
  ...
  // Update localStorage with current wishlist
  updateWishlistInLocalStorage() {
    localStorage.setItem('wishlist', JSON.stringify(this.wishlist));
  }
  filterBooks() {
    return this.books.filter((book: { volumeInfo: { [x: string]: string; }; }) => {
      const value = book.volumeInfo[this.searchKey]?.toLowerCase();
      return value && value.includes(this.searchQuery.toLowerCase());
    });
  }

  onSearchInputChange(event: Event) {
    const input = event.target as HTMLInputElement;
    this.searchQuerySubject.next(input.value);
  }
}
